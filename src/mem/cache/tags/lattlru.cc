...
LATTLRU::LATTLRU( unsigned _numSets,
        unsigned _blkSize,
        unsigned _assoc,
...
unsigned LATTLRU::curr_L_assoc()
{
	return 0;
}

unsigned LATTLRU::lookup_umon(int index, uint64_t tid)
{
	return umon_counters[tid][index];
}

unsigned LATTLRU::lookup_misses(uint64_t tid)
{
...
// label: 0->L, 1->H
CacheSet
LATTLRU::get_set( int setnum, uint64_t label, Addr addr ){
    CacheSet s = sets[label][setnum];
    return s;
}

int
LATTLRU::assoc_of_tc( int tcid ){
...
unsigned
LATTLRU::inc_size(uint64_t tid0, uint64_t tid1){
...
unsigned 
LATTLRU::dec_size(uint64_t tid0, uint64_t tid1){
...
LATTLRU::BlkType*
LATTLRU::get_evictBlk(unsigned tcid, unsigned index){
	return sets[tcid][index].blks[per_assoc[tcid]-1];
}

LATTLRU::BlkType*
LATTLRU::accessBlock(Addr addr, int &lat, int master_id, uint64_t tid)
{
...
LATTLRU::BlkType*
LATTLRU::findBlock(Addr addr, uint64_t tid)
{
    Addr tag = extractTag(addr);
    unsigned set = extractSet(addr);
    BlkType *blk;
...
void
LATTLRU::invalidateBlk(BlkType *blk, uint64_t tid)
{
    if (blk) {
        if (blk->isValid()) {
            tagsInUse--;
