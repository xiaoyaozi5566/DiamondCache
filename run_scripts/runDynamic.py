...
def diamond_static_cache():
    for cpu in cpus:
        for workload in multiprog4:
            ...
            p2 = workload[2]
            p3 = workload[3]
            filename = cpu + "_" + "diamond_static" + "_" + p0 + "_" + p1 + "_" + p2 + "_" + p3
            script = open(scriptgen_dir + "/run_" + filename, "w")
            command = "#!/bin/bash\n"
            command += "build/ARM/gem5.fast \\\n"
            command += "    --remote-gdb-port=0 \\\n"
        ...
        for workload in multiprog4:
            for H_min in H_mins:
                for threshold in thresholds:
                    ...
                    p2 = workload[2]
                    p3 = workload[3]
                    filename = cpu + "_" + "diamond_linear" + "_" + p0 + "_" + p1 + "_" + p2 + "_" + p3 + "_" + str(H_min) + "_" + str(int(threshold*100))
                    script = open(scriptgen_dir + "/run_" + filename, "w")
                    command = "#!/bin/bash\n"
                    command += "build/ARM/gem5.fast \\\n"
                    command += "    --remote-gdb-port=0 \\\n"
...
# count_phases()
# dumb_flush()
# diamond_static_cache()
# diamond_linear_cache()